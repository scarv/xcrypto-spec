...
\XCINSTR{xc.scatter.b}{rs3, rs1(rs2)}{
...
\XCINSTR{xc.scatter.h}{rs3, rs1(rs2)}{
  Perform a ``scattered'' store of 
  two  half-words
  from an $\GPR$ register
  into memory;
  ...
  $\MEM[*][addr_0+0]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs3}}]}{0}$ \;
  $\MEM[*][addr_0+1]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs3}}]}{1}$ \;
  $\MEM[*][addr_1+0]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs3}}]}{2}$ \;
  $\MEM[*][addr_1+1]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs3}}]}{3}$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.gather.b}{rd, rs1(rs2)}{
  ...
  The {\em high} bits of the destination register from $\XLEN$ to $32$
  are zeroed.
}{
  $addr_0                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs2}}]}{0}$ \;
  $addr_1                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[ 8]{\GPR[*][{\VERB[RV]{rs2}}]}{1}$ \;
...
\XCINSTR{xc.gather.h}{rd, rs1(rs1)}{
  Perform a  ``gathered''  load of 
  two  half-words
  from memory 
  into an $\GPR$ register;
