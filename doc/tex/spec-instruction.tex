...
\XCINSTR{xc.ld.bu}{crd, h, b, imm11(rs1)}{
  ...
  \VERB[RV]{h} is stored in the \VERB[RV]{cc} field.
  \VERB[RV]{b} is stored in the \VERB[RV]{cd} field.
}{
  $addr                                          \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
  $index                                         \ASN ( \VERB[RV]{cc} \CONS \VERB[RV]{cd} )$ \;
  $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{index  } \ASN \MEM[*][{addr  }]$ \;
  \If{$index = 0$}{
    $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{x} \ASN 0, \forall x \in \{1 \le x < \XCLEN/8$\} \;
  }
}
...
\XCINSTR{xc.ld.hu}{crd, h,    imm11(rs1)}{
  ...
  $addr                                          \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
  $index_0                                       \ASN ( \VERB[RV]{cc} \CONS 0 )$ \;
  $index_1                                       \ASN ( \VERB[RV]{cc} \CONS 1 )$ \;
  $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{index_0} \ASN \MEM[*][{addr+0}]$ \;
  $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{index_1} \ASN \MEM[*][{addr+1}]$ \;
  \If{$index_0 = 0$}{
    $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{x} \ASN 0, \forall x \in \{2 \le x < \XCLEN/8$\} \;
  }
}
...
\XCINSTR{xc.ld.w} {crd,        imm11(rs1)}{
  ...
  $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{x} \ASN 0, \forall x \in \{4 \le x < \XCLEN/8$\} \;
}
...
exceptions in the same way as the base RISC-V \VERB[RV]{lhu} instruction does.
\VERB[RV]{b0} refers to the first byte of the destination register to modify.
\VERB[RV]{b0} must be either $0$ or $2$, denoting the lower or upper halfword respectivley.
...
If \VERB[RV]{b0} is $1$ or $3$, an illegal instruction exception is raised.
}{
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $index \ASN \VERB[RV]{b0}$ \;
  $\INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{index  } \ASN \MEM[*][{addr  }]$ \;
...
exceptions in the same way as the base RISC-V \VERB[RV]{lw} instruction does.
}{
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
...
$\GPR[*][{\VERB[RV]{rs2}}]$.
If the resulting address is not halfword aligned, implementations must raise
exceptions in the same way as the base RISC-V \VERB[RV]{sh} instruction does.
...
\XCINSTR{xc.rngseed}{crs1}{
  Inject (or seed) $\XCLEN$-bits of entropy into the RNG, reading from an $\XCR$ register.
}{
...
\XCINSTR{xc.rngsamp}{crd}{
  Sample           $\XCLEN$-bits of entropy from the RNG, writing to   an $\XCR$ register.
}{
...
\XCINSTR{xc.scatter.b}{crd, crs2, rs1}{
  ...
  plus
  the $i$-th byte within an $\XCR$ register (the offset).
}{
  \For{$i = 0$ {\bf upto} $(XCLEN/8)-1$} {
      $addr_i                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[ 8]{\XCR[*][{\VERB[RV]{crs2}}]}{i}$ \;
      $\MEM[*][addr_i  ]                        \ASN \INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{i}$ \;
  }
}
...
\XCINSTR{xc.scatter.h}{crd, crs2, rs1}{
  ...
  \For{$i = 0$ {\bf upto} $(XCLEN/16)-1$} {
    $addr_i                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\XCR[*][{\VERB[RV]{crs2}}]}{i}$ \;
    $\MEM[*][addr_i+0]                        \ASN \INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{2*i}$ \;
    $\MEM[*][addr_i+1]                        \ASN \INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{2*i+1}$ \;
  }
}
...
\XCINSTR{xc.gather.b}{crd, crs2, rs1}{
  ...
  \For{$i = 0$ {\bf upto} $(XCLEN/8)-1$} {
      $addr_i                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[ 8]{\XCR[*][{\VERB[RV]{crs2}}]}{i}$ \;
      $\INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{i} \ASN \MEM[*][addr_i  ]$ \;
  }
}
...
\XCINSTR{xc.gather.h}{crd, crs2, rs1}{
  ...
  \For{$i = 0$ {\bf upto} $(XCLEN/16)-1$} {
      $addr_i                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\XCR[*][{\VERB[RV]{crs2}}]}{i}$ \;
      $\INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{2*i}   \ASN \MEM[*][addr_i  ]$ \;
      $\INDEX[ 8]{\XCR[*][{\VERB[RV]{crd}}]}{2*i+1} \ASN \MEM[*][addr_i+1]$ \;
  }
}
...
\XCINSTR{xc.ld.hiu}{crd, imm16}{
  ...
  $\VERB[RV]{imm16} = \VERB[RV]{imm11} \CONS \VERB[RV]{imm5}$ \;
  $\INDEX[16]{\XCR[*][{\VERB[RV]{crd}}]}{\XCLEN/16-1} \ASN \VERB[RV]{imm16}$ \;
}
...
\XCINSTR{xc.bop}{crd, crs1, crs2, lut8}{
  ...
  \VERB[RV]{lut8} &=& \RADIX{10001000}{2} &\Longrightarrow& f &=& y \AND z \\
  \VERB[RV]{lut8} &=& \RADIX{11101110}{2} &\Longrightarrow& f &=& y \IOR z \\
  \VERB[RV]{lut8} &=& \RADIX{01100110}{2} &\Longrightarrow& f &=& y \XOR z \\
  \VERB[RV]{lut8} &=& \RADIX{01100110}{2} &\Longrightarrow& f &=& y \XOR z \\
  \VERB[RV]{lut8} &=& \RADIX{10010110}{2} &\Longrightarrow& f &=& x \XOR y \XOR z \\
  \VERB[RV]{lut8} &=& \RADIX{01101010}{2} &\Longrightarrow& f &=& x \XOR y \AND z \\
  \end{array}
  \]
}{
  \For{$i = 0$ {\bf upto} $\XCLEN-11$} {
...
\XCINSTR{xc.ext}{crd, crs, start, length}{
  ...
  For an instruction word to decode as $\VERB[RV]{xc.ext}$,
  it is required that
...
\XCINSTR{xc.ins}{crd, crs, start, length}{
...
\XCINSTR{xc.lut}{crd, crs1, crs2, crs3}{
  ...
  $\XCR[*][{\VERB[RV]{crd}}]$.
  The LUT is specified by concatenating
  $
  ...
  \For{$i=0$ {\bf upto} $15$}{
    lut[i] = \INDEX[4]{
        {(\XCR[*][{\VERB[RV]{crs3}}]{\XCLEN-1 \RANGE 0} \CONS 
          \XCR[*][{\VERB[RV]{crs2}}]{\XCLEN-1 \RANGE 0})}
    }{i}
  }
  \For{$i=0$ {\bf upto} $\XCLEN/4-1$}{
...
\XCINSTR{xc.pperm.hx}{crd, crs1, b0, b1, b2, b3}{
  Permute nibbles of a half-word in some $\XCR$ register,
  storing the result in a half-word of another $\XCR$ register.
  Note that the index 
  $
