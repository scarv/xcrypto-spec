...
\XCINSTR{xc.ldr.bu}{rd(b0), rs1, rs2}{
  Load a      byte from memory
  into a specific      byte of an $\GPR$ register;
  Beyond this, note that
  1) if
     $\VERB[RV]{b0} =    0$,
     all other      bytes in the $\GPR$ register are set to zero,
     whereas
  2) if 
     $\VERB[RV]{b0} \neq 0$,
     no  other      bytes in the $\GPR$ register are modified.
}{
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $index \ASN \VERB[RV]{b0}$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index  } \ASN \MEM[*][{addr  }]$ \;
  \If{$index = 0$}{
    $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{1} \ASN 0$ \;
    $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{2} \ASN 0$ \;
    $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{3} \ASN 0$ \;
  }
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.ldr.hu}{rd(b0), rs1, rs2}{
  Load a half-word from memory
  into a specific half-word of an $\GPR$ register;
  Beyond this, note that
  1) if
     $\VERB[RV]{b0} =    0$,
     all other half-words in the $\GPR$ register are set to zero,
     whereas
  2) if 
     $\VERB[RV]{b0} \neq 0$,
     no  other half-words in the $\GPR$ register are modified.
  ...
  $index \ASN \VERB[RV]{b0}$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index  } \ASN \MEM[*][{addr  }]$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index+1} \ASN \MEM[*][{addr+1}]$ \;
  \If{$index = 0$}{
    $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{2} \ASN 0$ \;
    $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{3} \ASN 0$ \;
  }
...
\XCINSTR{xc.ldr.w}{rd, rs1, rs2}{
  ...
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{0} \ASN \MEM[*][{addr+0}]$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{1} \ASN \MEM[*][{addr+1}]$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{2} \ASN \MEM[*][{addr+2}]$ \;
  $\INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{3} \ASN \MEM[*][{addr+3}]$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.str.b}{rd(b0), rs1, rs2}{
  Store a specific      byte of an $\GPR$ register into memory.
}{
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $index \ASN \VERB[RV]{b0}$ \;
  $\MEM[*][{addr  }] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index}$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.str.h}{rd(b0), rs1, rs2}{
  Store a specific half-word of an $\GPR$ register into memory.
  If the effective address $addr$ is not half-word aligned, 
  the memory access {\em must} raise an exception in the same way
  as the base ISA 
  ...
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $index \ASN \VERB[RV]{b0}$ \;
  $\MEM[*][{addr+0}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index+0}$ \;
  $\MEM[*][{addr+1}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{index+1}$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.str.w}{rd, rs1, rs2}{
  Store                         an $\GPR$ register into memory.
  If the effective address $addr$ is not      word aligned, 
  the memory access {\em must} raise an exception in the same way
  as the base ISA 
  ...
  $addr  \ASN \GPR[*][{\VERB[RV]{rs1}}] + \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $\MEM[*][{addr+0}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{0}$ \;
  $\MEM[*][{addr+1}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{1}$ \;
  $\MEM[*][{addr+2}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{2}$ \;
  $\MEM[*][{addr+3}] \ASN \INDEX[8]{\GPR[*][{\VERB[RV]{rd}}]}{3}$ \;
}
...
\XCINSTR{xc.rngseed}{rs1}{
  Inject (or seed) $32$-bits of entropy into the RNG, reading from an $\GPR$ register.
}{
...
\XCINSTR{xc.rngsamp}{rd}{
  Sample           $32$-bits of entropy from the RNG, writing to   an $\GPR$ register.
}{
...
\XCINSTR{xc.scatter.b}{rd, rs2, rs1}{
  ...
  $\MEM[*][addr_0  ]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{0}$ \;
  $\MEM[*][addr_1  ]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{1}$ \;
  $\MEM[*][addr_2  ]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{2}$ \;
  $\MEM[*][addr_3  ]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{3}$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.scatter.h}{rd, rs2, rs1}{
  Perform a ``scattered'' store of 
  two  half-words
  from an $\GPR$ register
  into memory;
  ...
  $addr_0                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\GPR[*][{\VERB[RV]{rs2}}]}{0}$ \;
  $addr_1                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\GPR[*][{\VERB[RV]{rs2}}]}{1}$ \;
  $\MEM[*][addr_0+0]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{0}$ \;
  $\MEM[*][addr_0+1]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{1}$ \;
  $\MEM[*][addr_1+0]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{2}$ \;
  $\MEM[*][addr_1+1]                        \ASN \INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{3}$ \;
}

% -----------------------------------------------------------------------------

\XCINSTR{xc.gather.b}{rd, rs2, rs1}{
...
\XCINSTR{xc.gather.h}{rd, rs2, rs1}{
  Perform a  ``gathered''  load of 
  two  half-words
  from memory 
  into an $\GPR$ register;
  ...
  $addr_0                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\GPR[*][{\VERB[RV]{rs2}}]}{0}$ \;
  $addr_1                                   \ASN \GPR[*][{\VERB[RV]{rs1}}] + \INDEX[16]{\GPR[*][{\VERB[RV]{rs2}}]}{1}$ \;
  $\INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{0} \ASN \MEM[*][addr_0+0]$ \;
  $\INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{1} \ASN \MEM[*][addr_0+1]$ \;
  $\INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{2} \ASN \MEM[*][addr_1+0]$ \;
  $\INDEX[ 8]{\GPR[*][{\VERB[RV]{rd}}]}{3} \ASN \MEM[*][addr_1+1]$ \;
}
...
\XCINSTR{xc.bop}{rd, rs1, rs2, lut8}{

{\bf TODO:} Sort out how this instruction is encoded.

  Compute each $1$-bit sub-word (i.e.,    bit) of an $\XLEN$-bit result
...
\XCINSTR{xc.lut}{rd, rs1, rs2, rs3}{
  Compute each $4$-bit sub-word (i.e., nibble) of an $\XLEN$-bit result
  via application of a function
  \[
  ...
  $f = \GPR[*][{\VERB[RV]{rs3}}] \CONS \GPR[*][{\VERB[RV]{rs2}}]$ \;
  $t \ASN 0$ \;
  \For{$i = 0$ {\bf upto}  $3$}{
...
\XCINSTR{xc.pmul.l}{pw, rd, rs1, rs2}{
...
\XCINSTR{xc.pmul.h}{pw, rd, rs1, rs2}{
  Compute a   register-based packed multiplication,
  implying $\odot = \times$;
...
\XCINSTR{xc.pclmul.l}{pw, rd, rs1, rs2}{
...
\XCINSTR{xc.pclmul.h}{pw, rd, rs1, rs2}{
  Compute a   register-based packed multiplication in $\B{F}_2[\IND{x}]$ (or ``carry-less'' multiplication),
  implying $\odot = \otimes$;
  this instruction captures the ``high'' half, or $w$ MSBs of 
  each computed product
...
\XCINSTR{xc.psll.i}{pw, rd, rs1, cshamt}{
...
\XCINSTR{xc.psrl.i}{pw, rd, rs1, cshamt}{
  Compute an immediate-based packed left-shift,
  implying $\odot = \RSH$.
  Keep in mind that
  any out-of-range distance
...
\XCINSTR{xc.pror.i}{pw, rd, rs1, cshamt}{
...
\XCINSTR{xc.madd.3}{(rd2, rd1), rs1, rs2, rs3}{
  ...
  $
  \VERB[RV]{crdm} \mapsto \TUPLE{ \VERB[RV]{rd2}, \VERB[RV]{rd1} } .
  $
}{
  $t                          \ASN ( \GPR[*][{\VERB[RV]{rs1}}] \OP[64][u]{+}        \GPR[*][{\VERB[RV]{rs2}}] ) 
                                                                \OP[64][u]{+} \INDEX{\GPR[*][{\VERB[RV]{rs3}}]}{0}$ \;
  $\GPR[*][{\VERB[RV]{rd1}}] \ASN \INDEX{t}{31 \RANGE  0}$ \;
  $\GPR[*][{\VERB[RV]{rd2}}] \ASN \INDEX{t}{64 \RANGE 32}$ \;
}


% -----------------------------------------------------------------------------

\XCINSTR{xc.msub.3}{(rd2, rd1), rs1, rs2, rs3}{
...
\XCINSTR{xc.macc.1}{(rd2, rd1), rs1      }{
  Compute a   register-based multi-precision (or ``long'') accumulation,
  with the
  $\XLEN$-bit addend
  $\GPR[*][{\VERB[RV]{rs1}}]$
  accumulated into a $( 2 \cdot \XLEN )$-bit value formed by concatenating
  $\GPR[*][{\VERB[RV]{rd1}}]$
  and
  $\GPR[*][{\VERB[RV]{rd2}}]$.
  Note that, wrt. the encoding,
  ...
  $t                          \ASN ( \GPR[*][{\VERB[RV]{rd2}}] \CONS \GPR[*][{\VERB[RV]{rd1}}] ) \OP[64][u]{+} \GPR[*][{\VERB[RV]{rs1}}]$ \;
  $\GPR[*][{\VERB[RV]{rd1}}] \ASN \INDEX{t}{31 \RANGE  0}$ \;
  $\GPR[*][{\VERB[RV]{rd2}}] \ASN \INDEX{t}{64 \RANGE 32}$ \;
}
...
\XCINSTR{xc.mmul.3}{(rd2, rd1), rs1, rs2, rs3}{
  Compute a   register-based multi-precision (or ``long'') multiply-accumulate:
  the $\XLEN$-bit multiplicands
  $\GPR[*][{\VERB[RV]{rs1}}]$
  and
  $\GPR[*][{\VERB[RV]{rs2}}]$
  are first used to form a $( 2 \cdot \XLEN )$-bit product captured by
  $\GPR[*][{\VERB[RV]{rd1}}]$ (the LSBs)
  and
  $\GPR[*][{\VERB[RV]{rd2}}]$ (the MSBs),
  into which the $\XLEN$-bit addend
  $\GPR[*][{\VERB[RV]{rs3}}]$
  is accumulated.
  Note that, wrt. the encoding,
  $
  ...
  $t                          \ASN ( \GPR[*][{\VERB[RV]{rs1}}] \OP[32][u]{ \times} \GPR[*][{\VERB[RV]{rs2}}] ) 
                                                                \OP[64][u]{ +}      \GPR[*][{\VERB[RV]{rs3}}]$ \;
  $\GPR[*][{\VERB[RV]{rd1}}] \ASN \INDEX{t}{31 \RANGE  0}$ \;
  $\GPR[*][{\VERB[RV]{rd2}}] \ASN \INDEX{t}{64 \RANGE 32}$ \;
}
...
\XCINSTR{xc.aessub}{.\[enc,dec\]\[rot\] rd, rs1, rs2}{
  ...
  \eIf{$\VERB[RV]{rot}  = 0$}{
    $\GPR[*][{\VERB[RV]{crd0}}] \ASN                                          t_3 \CONS t_2 \CONS t_1 \CONS t_0 $ \;
  }{
    $\GPR[*][{\VERB[RV]{crd0}}] \ASN                                          t_2 \CONS t_1 \CONS t_0 \CONS t_3 $ \;
  }
}

\XCINSTR{xc.aesmix}{mode,      rd, rs1, rs2}{
  Per~\cite[Figures 5]{SCARV:TilGro:06}, 
  apply a (carefully structured) form of the AES 
  ...
  \eIf{$\VERB[RV]{mode} = 0$}{
    $\GPR[*][{\VERB[RV]{crd0}}] \ASN \SCOPE{\ID{AES}}{\ALG{Mix-Column}}     ( t_3 \CONS t_2 \CONS t_1 \CONS t_0 )$ \;
  }{
    $\GPR[*][{\VERB[RV]{crd0}}] \ASN \SCOPE{\ID{AES}}{\ALG{Mix-Column}}^{-1}( t_3 \CONS t_2 \CONS t_1 \CONS t_0 )$ \;
  }
}
...
\subsubsection{Class-$3.2$: SHA2 Instructions}
\label{sec:spec:instruction:3:2}

These instructions are designed to accelerate critical parts of the
SHA2 round functions.

\XCINSTR{xc.sha256.s0}{rd, rs1}{

}{
    $a \ASN \GPR[*][{\VERB[RV]{rs1}}]$\;
    $\GPR[*][{\VERB[RV]{rd}}] \ASN (a \RRT 7)\XOR(a \RRT 18) \XOR(a \RSH 3)$
}

\XCINSTR{xc.sha256.s1}{rd, rs1}{

}{
    $a \ASN \GPR[*][{\VERB[RV]{rs1}}]$\;
    $\GPR[*][{\VERB[RV]{rd}}] \ASN (a \RRT 17)\XOR(a \RRT 19) \XOR(a \RSH 10)$
}

\XCINSTR{xc.sha256.s2}{rd, rs1}{

}{
    $a \ASN \GPR[*][{\VERB[RV]{rs1}}]$\;
    $\GPR[*][{\VERB[RV]{rd}}] \ASN (a \RRT 2)\XOR(a \RRT 13) \XOR(a \RRT 22)$
}

\XCINSTR{xc.sha256.s3}{rd, rs1}{

}{
    $a \ASN \GPR[*][{\VERB[RV]{rs1}}]$\;
    $\GPR[*][{\VERB[RV]{rd}}] \ASN (a \RRT 6)\XOR(a \RRT 11) \XOR(a \RRT 25)$
}
