...
\XCINSTR{xc.ext}{crd, crs, start, length}{
  ...
  The $\VERB[RV]{start}$ argument is stored in the $\VERB[RV]{cs}$ field.
  The $\VERB[RV]{length}$ argument is stored in the $\VERB[RV]{cl}$ field.
  Note that in the case where $\VERB[RV]{cl} = 0$, this indicates
  a bitfield of length $1$.
  For an instruction word to decode as $\VERB[RV]{xc.ext}$,
  it is required that
  $\VERB[RV]{start} + \VERB[RV]{length}$ be less than the source register
  length.
}{
  $lo \ASN \VERB[RV]{cs}                $ \;
...
\XCINSTR{xc.ins}{crd, crs, start, length}{
  ...
  Note that in the case where $\VERB[RV]{cl} = 0$, this indicates
  a bitfield of length $1$.
  When $\VERB[RV]{start} + \VERB[RV]{length}$ is greater than the length
  of the source register, an illegal opcode exception must be thrown.
